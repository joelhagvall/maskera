#!/bin/sh
# v12 take 3 (v12c): decomposed-surname fix. Take 1 failed the gate on bare
# "Löfven" (4 gold-real misses); bisection showed the damage is the trim_vocab
# tokenization mismatch (trained on single-token rare names, inference sees
# them decomposed), and take 2 (recovery finetune, run_v12b.sh) proved a
# post-hoc finetune cannot restore it (trimmed teacher is equally blind).
# Fix at the data level: generate_data.mjs adds RARE_LAST (32 surnames
# verified to decompose under the full vocab) and a 8% bare-surname share in
# person(), so teacher AND student train the decomposed-capitalized-name
# pattern from the start. Mix otherwise identical to run_v12.sh.
# Gate stays on the QUANTIZED artifact (gold-real recall floor 0.90).
set -e
cd /Users/joelhagvall/Documents/GitHub/maskera/training
export PYTORCH_ENABLE_MPS_FALLBACK=1 TOKENIZERS_PARALLELISM=false
PY=.venv/bin/python
echo "[0/6] build data (synthetic + klintan + sucx + sic2 + massive + multiconer)..."
...
node audit_data.mjs
echo "[1/6] train teacher model-v12c (log: run_v12c-teacher.log)..."
$PY train.py model-v12c >run_v12c-teacher.log 2>&1
echo "[2/6] distill student-v12c (log: run_v12c-distill.log)..."
$PY distill.py 6 student-v12c model-v12c >run_v12c-distill.log 2>&1
echo "[3/6] trim vocab..."
$PY trim_vocab.py student-v12c student-v12c-trimmed 16000
echo "[4/6] export onnx..."
rm -rf student-v12c-onnx
$PY export_onnx.py student-v12c-trimmed student-v12c-onnx
echo "[5/6] q4 combo..."
$PY quantize_combo.py student-v12c-onnx
echo "[6/6] eval..."
...
const rec=createNerRecognizer({model:"student-v12c-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
// acronym + real-org + chat-register spot checks (v12c adds the leak-category
// probes: startup brand + multiword authority, NOT the eval entities)
...
JS
node _v12c.mjs || { rm -f _v12c.mjs; echo "[failed gate]"; exit 1; }
rm -f _v12c.mjs
echo "[done]"
