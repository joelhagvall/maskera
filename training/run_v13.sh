#!/bin/sh
# v13: DESIGNED decomposed-surname robustness, the v12 publish blocker.
# v12 passed every existing gate but leaked rare decomposed surnames in the
# target register ("hej jag heter tjulander..." unmasked) where v11 masks
# them; v11's ability was luck-of-the-mix. This round makes it a trained,
# gated property (docs/ROADMAP.md v13 item 1):
#   - NEW EVAL: eval/rare-surnames.txt (gen_rare_surname_eval.mjs), 294
#     chat-register sentences, 98 surnames verified to decompose under the
#     20k trim and absent from ALL training data. Baselines (2026-07-11,
#     masked-at-all): v11 94.9% (15 leaks), v12 90.5% (28 leaks).
#   - FIX: distill.py subword replacement (MASKERA_SUBWORD_DROPOUT): the
#     student trains on the trimmed-vocab tokenization, i.e. the exact token
#     sequences it faces after trim_vocab.py; the teacher keeps the full
#     tokenization (trimmed teacher is proven blind to decomposed names) and
#     the KL is word-aligned. p=1.0: for words inside the 20k vocab the
#     replacement is a no-op, and for the rest the decomposed form is the
#     ONLY form post-trim inference ever sees, so there is nothing to mix.
#   - Also: municipal "-avdelningen" AUTHORITIES entries (ROADMAP quick fix).
# Mix otherwise identical to run_v12.sh; trim stays at 20k (v12 take 4).
# Gates on the QUANTIZED artifact: gold-real recall floor 0.90 AND
# rare-surname masked recall STRICTLY ABOVE v11's, measured in this run.
set -e
...
echo "[1/8] train teacher model-v13 (log: run_v13-teacher.log)..."
$PY train.py model-v13 >run_v13-teacher.log 2>&1
echo "[2/8] trim teacher vocab -> dropout tokenizer (log: run_v13-trimtok.log)..."
...
$PY trim_vocab.py model-v13 model-v13-trimmed 20000 >run_v13-trimtok.log 2>&1
echo "[3/8] distill student-v13 with subword replacement (log: run_v13-distill.log)..."
MASKERA_SUBWORD_DROPOUT=1.0 MASKERA_DROPOUT_VOCAB=model-v13-trimmed \
  $PY distill.py 6 student-v13 model-v13 >run_v13-distill.log 2>&1
echo "[4/8] trim student vocab (20000, the v12 take-4 lesson)..."
$PY trim_vocab.py student-v13 student-v13-trimmed 20000
echo "[5/8] export onnx..."
rm -rf student-v13-onnx
$PY export_onnx.py student-v13-trimmed student-v13-onnx
echo "[6/8] q4 combo..."
$PY quantize_combo.py student-v13-onnx
...
const rec=createNerRecognizer({model:"student-v13-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
JS
node _v13.mjs || { rm -f _v13.mjs; echo "[failed gate]"; exit 1; }
rm -f _v13.mjs
echo "[8/8] rare-surname gate: v13 must BEAT v11 (docs/ROADMAP.md: not just tie)..."
cd /Users/joelhagvall/Documents/GitHub/maskera
MASKERA_MODEL_PATH="$PWD/training" MASKERA_MODEL=student-v13-onnx \
  node packages/ner/eval/benchmark-rare-surnames.mjs | tee training/run_v13-raresurnames-v13.log
...
const v13 = parse("training/run_v13-raresurnames-v13.log")
