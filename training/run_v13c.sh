#!/bin/sh
# v13 take 3 (v13c): support-register PER frames. Takes so far on the
# rare-surname gate (v11 94.9% masked / 15 leaks is the bar):
#   take 1 (run_v13.sh):  84.0% / 47. Subword replacement p=1.0 put the
#     decomposed ability in the fp32 weights (trimmed student tags
#     "tjulander"; fp32=q8=q4 within 1pp so quantization is innocent), but
#     continuation subtokens were unsupervised (-100) and the incoherent
#     B/B/I chains failed reconstruct()'s whole-word guard.
#   take 2 (run_v13b.sh): 92.9% / 21. Continuation labels (I- on every
#     non-first piece, KL still word-aligned via kl_mask) fixed the chains:
#     "hej jag heter tjulander" caught in q4, ALL-CAPS leaks 8 -> 2, and the
#     gold sets hit records (our 97.9, gold-real 96.6 F1 / 0.97 recall).
#     Remaining leaks cluster in support-register frames the templates never
#     taught (closers "mvh X", callbacks "be X återkomma", self-intros "det
#     är X här"): v11 caught those by luck of the mix, v12's MultiCoNER
#     O-prior removed the luck.
# take 3 teaches those FRAMES in generate_data.mjs with {PER} full/first
# names only (bare-surname slots stay banned, the v12c poison) and phrasings
# deliberately different from eval/rare-surnames.txt, so the eval keeps
# measuring full-name -> bare-decomposed-surname generalisation. Data
# changes, so teacher + dropout tokenizer are retrained; training mechanics
# are identical to take 2.
...
echo "[1/8] train teacher model-v13c (log: run_v13c-teacher.log)..."
$PY train.py model-v13c >run_v13c-teacher.log 2>&1
echo "[2/8] trim teacher vocab -> dropout tokenizer (log: run_v13c-trimtok.log)..."
...
$PY trim_vocab.py model-v13c model-v13c-trimmed 20000 >run_v13c-trimtok.log 2>&1
echo "[3/8] distill student-v13c with subword replacement (log: run_v13c-distill.log)..."
MASKERA_SUBWORD_DROPOUT=1.0 MASKERA_DROPOUT_VOCAB=model-v13c-trimmed \
  $PY distill.py 6 student-v13c model-v13c >run_v13c-distill.log 2>&1
echo "[4/8] trim student vocab (20000, the v12 take-4 lesson)..."
$PY trim_vocab.py student-v13c student-v13c-trimmed 20000
echo "[5/8] export onnx..."
rm -rf student-v13c-onnx
$PY export_onnx.py student-v13c-trimmed student-v13c-onnx
echo "[6/8] q4 combo..."
$PY quantize_combo.py student-v13c-onnx
...
const rec=createNerRecognizer({model:"student-v13c-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
JS
node _v13c.mjs || { rm -f _v13c.mjs; echo "[failed gate]"; exit 1; }
rm -f _v13c.mjs
echo "[8/8] rare-surname gate: v13 must BEAT v11 (docs/ROADMAP.md: not just tie)..."
cd /Users/joelhagvall/Documents/GitHub/maskera
MASKERA_MODEL_PATH="$PWD/training" MASKERA_MODEL=student-v13c-onnx \
  node packages/ner/eval/benchmark-rare-surnames.mjs | tee training/run_v13c-raresurnames-v13c.log
...
const v13 = parse("training/run_v13c-raresurnames-v13c.log")
