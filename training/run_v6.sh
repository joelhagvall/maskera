...
PY=.venv/bin/python
echo "[0/6] build data (synthetic + klintan real text)..."; node generate_data.mjs >/dev/null 2>&1; node convert_klintan.mjs >/dev/null 2>&1
echo "[1/6] train teacher model-v6..."; $PY train.py model-v6 >/dev/null 2>&1
echo "[2/6] distill student-v6..."; $PY distill.py 6 student-v6 model-v6 >/dev/null 2>&1
echo "[3/6] trim vocab..."; $PY trim_vocab.py student-v6 student-v6-trimmed 16000
echo "[4/6] export onnx..."; rm -rf student-v6-onnx; $PY export_onnx.py student-v6-trimmed student-v6-onnx
echo "[5/6] q4 combo..."; $PY quantize_combo.py student-v6-onnx
echo "[6/6] eval..."
...
const rec=createNerRecognizer({model:"student-v6-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
// v5 measures 0.95 here; the 2026-07-04 ORG-collapse run measured 0.74.
...
JS
node _v6.mjs || { rm -f _v6.mjs; echo "[failed gate]"; exit 1; }
rm -f _v6.mjs
echo "[done]"
