#!/bin/sh
# v12 candidate: category-level ORG gazetteer round. Every remaining v11 leak
# was ORG (startup brands + multiword authorities), so:
#   - generate_data.mjs grew AUTHORITIES, smallBiz() and ~60 startup brands
#     (category instances only, eval entities excluded), and
#   - MultiCoNER v2 sv joins the mix (4.9k gold all-lowercase rows,
#     MULTICONER_SHARE 0.5, poison classes dropped, see convert_multiconer.mjs)
#     to reinforce lowercase ORG recall from the data side.
# Mix: synthetic 24k + klintan 8.5k + SUCX 14.6k + SIC2 1.1k + MASSIVE 5.1k
#      + MultiCoNER 4.9k ≈ 58k rows.
# Gate stays on the QUANTIZED artifact (gold-real recall floor 0.90; v11 = 0.93).
set -e
...
node audit_data.mjs
echo "[1/6] train teacher model-v12 (log: run_v12-teacher.log)..."
$PY train.py model-v12 >run_v12-teacher.log 2>&1
...
echo "[5/6] q4 combo..."
$PY quantize_combo.py student-v12-onnx
echo "[6/6] eval..."
...
const rec=createNerRecognizer({model:"student-v12-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
// acronym + real-org + chat-register spot checks (v12 adds the leak-category
...
JS
node _v12.mjs || { rm -f _v12.mjs; echo "[failed gate]"; exit 1; }
rm -f _v12.mjs
echo "[done]"
