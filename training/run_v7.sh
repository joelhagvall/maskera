...
echo "[1/6] train teacher model-v7..."; $PY train.py model-v7 >/dev/null 2>&1
echo "[2/6] distill student-v7..."; $PY distill.py 6 student-v7 model-v7 >/dev/null 2>&1
echo "[3/6] trim vocab..."; $PY trim_vocab.py student-v7 student-v7-trimmed 16000
echo "[4/6] export onnx..."; rm -rf student-v7-onnx; $PY export_onnx.py student-v7-trimmed student-v7-onnx
echo "[5/6] q4 combo..."; $PY quantize_combo.py student-v7-onnx
echo "[6/6] eval..."
...
const rec=createNerRecognizer({model:"student-v7-onnx",localModelPath:resolve(process.cwd(),"../../training")+"/",allowLocalModels:true,allowRemoteModels:false,dtype:"q4",device:"cpu",labelMap:g=>g})
await rec.ready
...
JS
node _v7.mjs || { rm -f _v7.mjs; echo "[failed gate]"; exit 1; }
rm -f _v7.mjs
echo "[done]"
