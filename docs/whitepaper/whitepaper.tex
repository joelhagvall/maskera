...
\usepackage[margin=32mm,top=30mm,bottom=32mm]{geometry}
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{array}
\usepackage{tikz}
...
\large\textmd{maskera: deterministic rules and a 40\,MB on-device NER model\\that mask PII before text reaches an LLM, a log, or analytics}}
...
\date{July 5, 2026 \quad\textperiodcentered\quad Whitepaper v1.3}

\begin{document}
\maketitle

\begin{abstract}
\noindent maskera is an open-source (MIT) library that redacts Swedish personally
identifiable information locally, in the browser or in the caller's own server
process, before text is sent anywhere. Structured identifiers (personnummer,
organisation numbers, phone numbers, bank account references, and others) are
caught by deterministic, checksum-validated rules. Names, places,
organisations, and street addresses in free text are caught by a purpose-built
Swedish NER model of 40\,MB, small enough to run where the text already is,
...
values is held locally by the caller. Measured on a small independent Swedish gold set, the
...
shaded boxes run inside the caller's environment.
...
genitive forms. The model is 40\,MB and runs in the browser (WebGPU or WASM)
and in Node.
...
\begin{itemize}
  \item \textbf{Model weights} ($\sim$40\,MB), downloaded once from the
...
Self-hosted model & caller's own origin only & none & production \\
...
the only things ever fetched are the static model and runtime files.
...
\begin{itemize}
  \item \textbf{Base model:} \pkg{KBLab/}\allowbreak\pkg{bert-base-swedish-cased},
        ...
        student model quantised to a 40\,MB ONNX artifact.
  \item \textbf{Training data:} approximately 24{,}000 synthetic,
        ...
        repository and contains no real personal data), plus approximately
        6{,}900 real news sentences from the Swedish NER Corpus, a public,
        openly licensed research dataset of already-published text about
        public figures.
...
This openness matters both for the caller's own GDPR assessment of the
component and for the transparency and documentation duties that follow from
the EU AI Act.

\section{Measured quality}

All numbers below were measured on \textbf{2026-07-04} against the published
artifact (\pkg{maskera@0.4.1}, q4 quantisation) using \textbf{exact character
spans}, the strict criterion under which both start and end offsets must
match. The canonical, always-current document is \pkg{docs/BENCHMARKS.md} in
...
\midrule
\textbf{maskera} (runs in the browser) & \textbf{40\,MB} & 0.98 & \textbf{0.96} \\
...
The full tables, all-lowercase results, method notes, and known exceptions
are in \pkg{BENCHMARKS.md}. One known exception is reported openly: on text
entirely without capital letters, KBLab's lowermix model leads (typed $F_1$
0.90 versus maskera's 0.86). None of the alternative models detects street
addresses or fits in a web application.
...
visit adds the network download of the $\sim$40\,MB model; returning
visitors read it from the browser cache.
...
Browser, WebGPU & 0.21\,s & 3.7 / 4.6\,ms & opt-in, slower first run \\
...
In practice: masking a chat message costs tens of milliseconds in the
browser and single-digit milliseconds on a server, and the deterministic
rule layer is effectively free. The measurement scripts live in the
repository, under \pkg{packages/ner/eval/} and \pkg{apps/demo/scripts/}.
...
\begin{itemize}
  \item \textbf{Data minimisation (Art.\ 5(1)(c) GDPR).} maskera's function
        ...
        before the API call is never processed by the recipient.
  ...
  \item \textbf{Third-country transfers (Chapter V GDPR).} Personal data
        that is never sent to a vendor outside the EU/EEA needs no transfer
        mechanism. Masking before the call shrinks the transfer question
        accordingly; it does not replace the caller's analysis of whatever
        is still sent.
...
\begin{itemize}
  \item \textbf{The model misses things.} The rule layer is deterministic and
        ...
        will sometimes slip through. On independent text the leakage is low
        (1 of 58 entities) but not zero.
  ...
  \item \textbf{The independent test set is small.} 22 sentences give a
        ...
        exists for the target domains of support, healthcare, and legal text.
...
\begin{itemize}
  ...
  \item \textbf{The numbers:} canonical in \pkg{docs/BENCHMARKS.md}, dated,
        and reproducible with documented commands against the published
        model.
  ...
  \item \textbf{The network claims:} verifiable in the browser's developer
        tools against the live demo at \url{https://maskera.dev}.
\end{itemize}
...
As of the artifact above, three gold entities across both corpora have no
overlapping detection; they are published here deliberately. In each
sentence every other entity is caught. On provenance: the two curated
sentences are hand-authored test cases with invented names, and the
gold-real sentence is verbatim, already-published Wikipedia prose about
public figures. No user data and no real private persons appear below.

\begin{table}[h]
\centering
\begin{tabular}{p{62mm}llp{34mm}}
\toprule
...
``hejhej det \"ar fatima igen, h\"or av dig n\"ar du kan'' & fatima & PER &
lowercase text interacting with name rarity (see below) \\
``Den 6 mars 2018 bes\"okte L\"ofven Vita huset och hade sitt f\"orsta
officiella m\"ote med [\dots]'' & Vita huset & LOC &
metonymic building name \\
\bottomrule
\end{tabular}
\end{table}

The second row deserves a sharper reading than ``no casing cues'':
capitalised ``Fatima'' \emph{is} caught, and so is lowercase ``anna''. The
failure is the combination of lowercase text and a name further from the
training distribution, i.e.\ robustness across name origins in casing-free
text. That gap is stated openly and targeted: the next data round adds
lowercase augmentation, and the project's gold-set plan requires invented
names spanning many origins, not only Swedish ones.

These are also why the rule layer exists and why leakage, not $F_1$, is the
gated metric: the failure modes are visible, characterised, and tracked in
the public regression corpus.

\vfill
\noindent\rule{\textwidth}{0.4pt}\\[0.3em]
{\footnotesize maskera \textperiodcentered{} whitepaper v1.3
\textperiodcentered{} July 5, 2026 \textperiodcentered{} Joel H\"agvall
\textperiodcentered{} maskera.dev \textperiodcentered{}
github.com/joelhagvall/maskera. Current numbers always live in
docs/BENCHMARKS.md; where this document disagrees, that file wins. This
