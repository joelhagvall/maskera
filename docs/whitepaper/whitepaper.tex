...
\date{July 16, 2026 \quad\textperiodcentered\quad Whitepaper v1.9}
...
\begin{itemize}
  ...
  \item \textbf{Training data:} approximately 25{,}000 synthetic,
        template-generated Swedish sentences (the generator is public in the
        repository and contains no real personal data), plus six public,
        openly licensed (CC~BY~4.0) corpora of already-published text
        (approximately 52{,}000 sentences in total): real news sentences
        from the Swedish NER Corpus, a weighted sample of SUCX~3.0
        (Spr\aa kbanken/KBLab, balanced genres), the Swedish subset of
        Amazon's MASSIVE corpus (lowercase chat-register utterances,
        professionally localised), SIC2 (Spr\aa kbanken, informal blog
        text), a class-audited sample of MultiCoNER~v2 Swedish (lowercase
        encyclopedic sentences), and an ensemble pseudo-labelled sample of
        Flashback/Familjeliv forum text (Spr\aa kbanken research corpora;
        pseudo-labels broaden the informal register, while every published
        quality number is measured on human-labelled sets).
...
All numbers below were measured on \textbf{2026-07-16} against the published
artifact (\pkg{maskera@0.6.1}, q4 quantisation, the v15 training round) using
...
Leakage & 0.0\% & entities missed entirely, 0 of 204 \\
\bottomrule
\end{tabular}
\end{table}
...
Metric & Result & Meaning \\
\midrule
Precision & 94.7\% & of the model's detections, how many were correct \\
Recall & 93.1\% & of the real entities, how many were found \\
Span $F_1$ & 93.9\% & harmonic mean, label-agnostic \\
...
Measured \textbf{2026-07-16} on the same q4 artifact: redaction recall is
...
and the exact span including the house number is recovered on \textbf{21 of
21} addresses (\emph{Sveavägen 44}, not just \emph{Sveavägen}). This
release's one documented exception also lives on this corpus: the ordinary
word \emph{Festen} is tagged as a person name in one distractor sentence, a
single harmless over-redaction (nothing leaks).
...
there: in the chat/support register maskera targets, the v15 round is the
strongest measured (99.3\% of rare out-of-training surnames masked, versus
98.3\% for the previous release, on a dedicated 294-sentence eval), while on
lowercased encyclopedic prose the case-robust KBLab lowermix model still
leads on raw redaction recall. None of the alternative models detects street
...
\begin{itemize}
  ...
  \item \textbf{The model:} published openly on Hugging Face
        ...
        {\footnotesize\texttt{ca6b4a66d199b0edfb397c39e7ffd25a55320dd4148bb9aed8f1f16ab007c727}};\\
...
As of the artifact above, one gold entity has no overlapping detection (a
genuine miss) and one ordinary word is over-redacted; both are published
here deliberately, and in each sentence every other real entity is caught.
Provenance: the gold-real sentence is verbatim, already-published
Wikipedia prose about public figures; the over-redacted sentence is a
hand-authored test case with an invented name. No user data and no real
private persons appear below.
...
``Festen \"ar hemma hos Oskar p\aa\ Linn\'egatan 52, fj\"arde v\aa ningen.''
& (none) & --- & over-redaction: the ordinary word \emph{Festen} is tagged
as a person name; nothing leaks \\
\bottomrule
\end{tabular}
\end{table}

The metonymic ``Vita huset'', fixed in v11, regressed back in v13 and
stays. The \emph{Festen} over-flag is new in v15 and is this release's one
documented exception: a harmless over-redaction accepted in exchange for
the round's fixes. Historically: bare ``L\"ofven'' and the all-caps
``VIKTIGT: RING LARS NORDSTR\"OM OMG\AA ENDE IDAG.'' were fixed by v13;
the sentence-initial ``Klarna'' (the curated classic since v5) and the
lowercase-encyclopedic register gap carried by v14 (independent corpus
forced lowercase, now 51 of 58 covered, back at the v11 level) were both
fixed by v15's balanced class replay; see \pkg{BENCHMARKS.md}. These misses
are why leakage, not $F_1$, is the gated metric: every failure mode is
visible and tracked in the public regression corpus.
...
{\footnotesize\raggedright maskera \textperiodcentered{} whitepaper v1.9
\textperiodcentered{} July 16, 2026 \textperiodcentered{} Joel H\"agvall
