...
\date{July 5, 2026 \quad\textperiodcentered\quad Whitepaper v1.1}
...
including inside a browser tab. No text leaves the device, no telemetry is
collected, and the mapping that restores masked values never leaves the
caller's environment. Measured on independent Swedish text, the pipeline flags
98\% of all PII entities and achieves the highest typed $F_1$ of every public
Swedish NER model tested, at less than a tenth of their size. maskera is a
...
\textbf{The caller's text never leaves the device.} All redaction runs
...
\begin{itemize}
  ...
  \item \textbf{The model:} published openly on Hugging Face
        (\pkg{joelhagvall/maskera-sv-ner}) with a model card and artifact
        hash.
...
{\footnotesize maskera \textperiodcentered{} whitepaper v1.1
