...
All numbers below were measured on \textbf{2026-07-14} against the published
artifact (\pkg{maskera@0.6.0}, q4 quantisation, the v14 training round) using
...
Recall & 99.5\% & of the real entities, how many were found \\
Span $F_1$ & 99.5\% & harmonic mean, label-agnostic \\
...
Metric & Result & Meaning \\
\midrule
Precision & 96.5\% & of the model's detections, how many were correct \\
Recall & 94.8\% & of the real entities, how many were found \\
Span $F_1$ & 95.7\% & harmonic mean, label-agnostic \\
...
Measured \textbf{2026-07-14} on the same q4 artifact: redaction recall is
...
there: in the chat/support register maskera targets, the v14 round is the
strongest measured (98.3\% of rare out-of-training surnames masked, versus
...
Both misses are unchanged from the previous release; the v14 training round
(pseudo-labeled informal Swedish plus targeted frames) moved neither.
Historically: bare ``L\"ofven'' and the all-caps ``VIKTIGT: RING LARS
NORDSTR\"OM OMG\AA ENDE IDAG.'' were fixed by v13, while the metonymic
``Vita huset'', fixed in v11, regressed back in v13 and stays. One register
gap is documented alongside: on the independent corpus forced lowercase,
coverage is 50 of 58 entities (up from the previous release's 48, still
below v11's 51; bare lowercase surnames in declarative prose), accepted as
a documented gate exception while both large lowercase benchmarks improved;
see \pkg{BENCHMARKS.md}. These misses are why leakage, not $F_1$, is the
