...
Measured \textbf{2026-07-07} on the same q4 artifact: redaction recall is
\textbf{100\%} (every address masked) with \textbf{zero leaks}, and address
precision is 95.5\%. The one weakness is the span boundary: on 17 of 21 the
model tags the street name but drops the house number (\emph{Sveavägen}, not
\emph{Sveavägen 44}). Because a bare house number is far less identifying than
the street, leakage stays at zero, but tightening the address span to include
the trailing number is the clear next training step for this class.
...
lowercase text plus a rare name (see below) \\
