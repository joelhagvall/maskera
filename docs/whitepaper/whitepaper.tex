...
All numbers below were measured on \textbf{2026-07-10} against the published
artifact (\pkg{maskera@0.4.5}, q4 quantisation, the v11 training round) using
...
\midrule
Precision & 97.6\% & of the model's detections, how many were correct \\
Recall & 98.5\% & of the real entities, how many were found \\
Span $F_1$ & 98.0\% & harmonic mean, label-agnostic \\
...
Metric & Result & Meaning \\
\midrule
Precision & 86.7\% & of the model's detections, how many were correct \\
Recall & 89.7\% & of the real entities, how many were found \\
Span $F_1$ & 88.1\% & harmonic mean, label-agnostic \\
...
Measured \textbf{2026-07-10} on the same q4 artifact: redaction recall is
...
\midrule
\textbf{maskera} (runs in the browser) & \textbf{40\,MB} & 1.00 & \textbf{0.94} \\
...
capital letters, previously maskera's known weak register, the v11 training
round closed the gap to the case-robust leader: KBLab's lowermix model
measures typed $F_1$ 0.90 versus maskera's 0.89, with equal redaction
recall (0.97). None of the alternative models detects street addresses or
fits in a web application.
...
sentence every other entity is caught. Provenance: the two curated
sentences are hand-authored test cases with invented names; the gold-real
sentence is verbatim, already-published Wikipedia prose about public
figures. No user data and no real private persons appear below.
...
``VIKTIGT: RING LARS NORDSTR\"OM OMG\AA ENDE IDAG.'' & LARS NORDSTR\"OM & PER &
long all-caps sentence; the shorter ``RING LARS NORDSTR\"OM'' is caught \\
``Den 6 mars 2018 bes\"okte L\"ofven Vita huset och hade sitt f\"orsta
officiella m\"ote med [\dots]'' & L\"ofven & PER &
bare surname without a first name \\
\bottomrule
\end{tabular}
\end{table}

Two previously published misses were fixed by the v11 training round
(real informal-register corpora) and moved out of this table: lowercase
``fatima'' (a name-origin robustness gap in casing-free text) and the
metonymic ``Vita huset''. These misses are why leakage, not $F_1$, is the
gated metric: every failure mode is visible and tracked in the public
regression corpus.
