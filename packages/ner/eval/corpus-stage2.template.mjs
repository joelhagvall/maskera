 ...
 *      then annotate every PERSON / LOCATION / ORGANIZATION / ADDRESS span by
 *      its exact substring. V19 predicts street addresses as its fourth
 *      free-text class. Annotate BEFORE running the model, so gold isn't
 *      anchored to model output (GOLD_SET_PLAN.md "Annotation protocol").
 ...
 *   - Labels: PERSON | LOCATION | ORGANIZATION | ADDRESS. Structured values
 *     such as personnummer and phone numbers remain rule-layer distractors,
 *     but v19's street-address spans are part of the free-text model eval.
 ...
 * @property {"support"|"healthcare"|"authority"|"everyday"} register
 * @property {string} collected       ISO date the message was written
 * @property {boolean} secondReviewed whether a second blind annotator reviewed the row
 */

// These are declarations about the collection process, not facts inferred by
// code. The validator makes every claim explicit and auditable before freeze.
export const provenance = {
  ...
  // Add only when the 200+ row corpus is frozen before model evaluation:
  // frozenAt: "2026-08-09",
}
...
export const corpus = [
  ...
  },
  {
    ...
    entities: [
      { value: "maskeragatan 148", label: "ADDRESS" },
  ...
  {
    text: "hej det är mohammed igen, min bror aleksander är inlagd på fiktivsjukhuset",
    entities: [
      { value: "mohammed", label: "PERSON" },
      { value: "aleksander", label: "PERSON" },
      { value: "fiktivsjukhuset", label: "ORGANIZATION" },
    ],
    writer: "W2",
    register: "healthcare",
